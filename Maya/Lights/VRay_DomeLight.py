...
# create_vray_dome_light(EXAMPLE["file_path"])
# -------------------------------------------------------------------------------

def create_vray_dome_light(hdri_path):
    """
    Creates a V-Ray dome light with the given HDRI.
    If a VRayLightDomeShape is already selected, updates its texture instead.
    """
    ...
    # Check if a V-Ray dome light is already selected
    for each in cmds.ls(selection=True):
        try:
            shape = cmds.listRelatives(each, shapes=True)[0]
            if cmds.objectType(shape) == "VRayLightDomeShape":
                cmds.setAttr(shape + ".useDomeTex", 1)
                connections = cmds.listConnections(shape + ".domeTex") or []
    ...
    dome_light = cmds.rename(dome_light, "VRayLightDome")
    cmds.setAttr(dome_light + ".useDomeTex", 1)
    ...
    cmds.connectAttr(file_node + ".outColor", dome_light + ".domeTex")
